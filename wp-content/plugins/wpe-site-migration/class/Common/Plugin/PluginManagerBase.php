...
class PluginManagerBase
{
    ...
    public function template_uploads_not_writable($templates)
    {
        ...
        if ($reminder_links) {
            $templates[$reminder_notice_name] = [
                'message' => sprintf(__('<strong>Uploads directory not writable</strong> &mdash; the following directory is not writable: <code>%s</code>. Update the file permissions for this folder to enable backups and export migrations. <a href="https://deliciousbrains.com/wp-migrate-db-pro/doc/uploads-folder-permissions/" target="_blank">More information</a>.<br><br>'), Filesystem::get_upload_info('path')),
                'link'    => $reminder_links,
        ...
        if (false !== ($deactivated_notice_id = get_transient(WPMDB_DEACTIVATED_NOTICE_ID_TRANSIENT))) {
            switch($deactivated_notice_id) {
                ...
                case WPMDB_DEACTIVATED_FOR_WPESM_ID:
                    $message = __("WP Engine Site Migration is powered by WP Migrate and shares some of the same functionality. In order to prevent conflicts between the two plugins, WP Migrate has been deactivated.", 'wp-migrate-db');
                    break;
                case WPESM_DEACTIVATED_FOR_WPMDB_ID:
                    $message = __("WP Engine Site Migration is powered by WP Migrate and shares some of the same functionality. In order to prevent conflicts between the two plugins, WP Engine Site Migration has been deactivated.", 'wp-migrate-db');
                    break;
                default:
    ...
    private function update_profiles()
    {
        foreach ([WPMDB_SAVED_PROFILES_OPTION, WPMDB_RECENT_MIGRATIONS_OPTION] as $profile_type) {
            $profiles = $profile_type === WPMDB_SAVED_PROFILES_OPTION ? $this->assets->get_saved_migration_profiles()
                : $this->assets->get_recent_migrations(get_site_option($profile_type));
            foreach ($profiles as $profile) {

                $loaded_profile = $this->profile_manager->get_profile_by_id($profile_type === WPMDB_RECENT_MIGRATIONS_OPTION ? 'unsaved' : $profile_type, $profile['id']);
