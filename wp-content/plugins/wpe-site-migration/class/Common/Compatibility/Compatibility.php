...
class Compatibility
{
    ...
    public function wpmdbc_tgmpa_compatibility()
    {
        ...
        if (defined('DOING_AJAX') && DOING_AJAX && isset($_POST['action']) && false !== strpos($_POST['action'],
                'wpmdb')) {
            $remove_function = true;
        }
    ...
    public function wpmdbc_include_plugins($plugins)
    {
        ...
        foreach ($plugins as $key => $plugin) {
            if (true === $this->wpmdbc_plugin_in_default_whitelist($plugin,
                    $default_whitelist) || isset($whitelist_plugins[$plugin])) {
                continue;
            }
    ...
    public function wpmdbc_is_wpmdb_background_migration_request()
    {
        if (
            ...
            ! empty($_GET['action']) &&
            (
                false !== strpos($_GET['action'], 'wpmdb__') ||
                false !== strpos($_GET['action'], 'wpesm__')
            )
        ) {
            return true;
        }
    ...
    public function wpmdbc_get_whitelist_plugins($plugins)
    {
        ...
        $option = in_array('wpe-site-migration/wpe-site-migration.php', $plugins) ? 'wpesm_settings' : 'wpmdb_settings';
